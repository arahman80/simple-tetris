...
inline static BOOL check_shift(game_state_t *state, I8 dist);
inline static U0 perform_shift(game_state_t *state, I8 dist);
inline static U0 copy_rows(game_state_t *state, I8 dist);
inline static U0 clear_vacated_rows(game_state_t *state, I8 dist);

BOOL v_shift(game_state_t *state, I8 dist, BOOL forced) {
  if (dist == 0) {
    return TRUE;
  }
...
inline static U0 determine_loop_bounds(I8 *start, I8 *end, I8 *step, I8 dist) {
  if (dist > 0) {
